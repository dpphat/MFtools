...
writehds <- function(hds, ofl){
    ...
    NTTS <- hds$TIME %>% data.frame() %>% dplyr::distinct() %>% nrow()
    NLAY <- hds$LAY %>% data.frame() %>% dplyr::distinct() %>% nrow()
    NC <- hds$COL %>% data.frame() %>% dplyr::distinct() %>% nrow()
    NR <- hds$ROW %>% data.frame() %>% dplyr::distinct() %>% nrow()
