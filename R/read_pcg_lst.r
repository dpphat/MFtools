...
read_pcg_lst <- function(LISTFL){
    ...
    hdchng_array <- linin[seq(HDCHNG_LOC, HDCHNG_LOC_END, by = 2)] %>% 
                    ...
                    strsplit("\\s+") %>% 
                    unlist()
    IN_OUT <- hdchng_array[seq(1, length(hdchng_array), by = 2)] %>% as.integer()
    HDCHNG_VAL <- hdchng_array[seq(2, length(hdchng_array), by = 2)] %>% as.numeric()
    hdchng_cell_array <- linin[seq(HDCHNG_LOC + 1, HDCHNG_LOC_END, by = 2)] %>% 
                         str_trim() %>% 
                         strsplit(")") %>% 
                         unlist()
    HDCHNG_CELL <- as_tibble(hdchng_cell_array %>% 
                             gsub("^.*\\:", "", .)) %>% 
                             separate(value, into = c("ITER", "LAYER", "ROW", "COL"), sep = "[^[:digit:]]+", convert = TRUE, extra = "drop") %>%
